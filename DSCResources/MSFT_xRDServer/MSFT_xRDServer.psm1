...
function Get-TargetResource
{
    ...
    write-verbose `
        "Getting list of servers of type '$Role' from '$ConnectionBroker'..."    
    #{
    ...
    {
        write-verbose `
            "Found $($servers.Count) '$Role' servers in the deployment, now looking for server named '$Server'..."
        ...
        if ($Server -in $servers.Server)
        {
            ...
            {
                write-verbose `
                    "the role is '$Role', querying RDS Gateway configuration..."

                $config = Get-RDDeploymentGatewayConfiguration `
                    -ConnectionBroker $ConnectionBroker

                if ($config)
                {
                    write-verbose `
                        "RDS Gateway configuration retrieved successfully..."
                    $result.GatewayExternalFqdn = $config.GatewayExternalFqdn
                    Write-verbose `
                        ">> GatewayExternalFqdn: '$($result.GatewayExternalFqdn)'"
                } 
            }
        }
        else
        {
            write-verbose `
                "The server '$Server' is not in the deployment as '$Role' yet."
        }
    ...
    {
        write-verbose `
            "No '$Role' servers found in the deployment on '$ConnectionBroker'."
...
function ValidateCustomModeParameters
{
    ...
    if ($Role -eq 'RDS-Gateway')
    {
        ...
        $nulls = $null
        $nulls = $customParams.getenumerator() | 
        Where-Object { $_.value -eq $null  }

        if ($nulls.count -gt 0)
        {
            $nulls | 
            ForEach-Object { write-verbose ">> '$($_.Key)' parameter is empty" }

            write-warning `
                "[PARAMETER VALIDATION FAILURE] i'm gonna throw, right now..."

            throw (
                    "Requested server role 'RDS-Gateway', you must pass in the 'GatewayExternalFqdn' parameter."
            )
        }
    }
    else
    {
        ...
        if ($parametersWithValues.count -gt 0)
        {
            ...
            ForEach-Object { 
                write-verbose `
                    ">> '$($_.Key)' was specified, the value is: '$($_.Value)'"
            }

            Write-Warning `
                ("[WARNING]: Requested server role is '$Role', the following parameter can only be used with server role 'RDS-Gateway': " + 
...
function Set-TargetResource
{
    ...
    Write-Verbose `
        "Adding server '$($Server.ToLower())' as $Role to the deployment on '$($ConnectionBroker.ToLower())'..."
    ...
    #{
        if (
            $Role -eq 'RDS-Licensing'  -or  
            $Role -eq 'RDS-Gateway' -or 
            $Role -eq 'RDS-WEB-ACCESS')
        {
            ...
            elseif (
                $e.count -eq 2 -and 
                $e[0].FullyQualifiedErrorId -eq 'CommandNotFoundException'
            )
            {
